...
class TurnosController {
  async getByPaciente(req, res) {
    ...
    await turnoService
      ...
      .catch((error) => {
        res.status(404).json({
          error: error.message || "No se encontraron turnos para el paciente",
        });
      });
  }
  ...
  async delete(req, res) {
    ...
    await turnoService
      ...
      .catch((error) => {
        res
          .status(404)
          .json({ error: error.message || "Error al cancelar el turno" });
      });
  }
  ...
  async create(req, res) {
    ...
    await turnoService
      ...
      .then((nuevoTurno) => {
        res
          .status(201)
          .json({ message: "Turno creado", idTurno: nuevoTurno.id });
      })
      .catch((error) => {
        res
          .status(400)
          .json({ error: error.message || "Error al crear el turno" });
      });
  }
}
